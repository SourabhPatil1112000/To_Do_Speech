...
class App extends Component {
	...
	handleSpeechRecognition = () => {
		...
		recognition.onresult = event => {
			...
			this.setState({ speechInput: transcript }, () => {
				// Once transcript is received, add task
				this.addAudioTask();
			});
		};
	...
	addAudioTask = () => {
		...
		this.setState(previousState => {
			...
			const newTask = {
				id: taskId,
				title: speechInput, // Use speechInput instead of input value
				status: 'To Do'
			};
			items.push(newTask);
			return {
				...
				submitDisabled: true,
				taskIdCounter: taskId,
				speechInput: '', // Clear speechInput after adding task
			};
		}, function stateUpdateComplete() {
			this.updateLocalStorageItems(this.state.items);
			this.updateTaskCounter(this.state.taskIdCounter);
